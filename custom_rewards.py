...
class SpeedKickoffReward(RewardFunction):
    def __init__(self):
        super().__init__()
    ...
    def get_reward(self, player: PlayerData, state: GameState, previous_action: np.ndarray) -> float:
        if np.all(state.ball.position[:2] == [0, 0]) and np.all(state.ball.linear_velocity == [0, 0, 0]):
            opponent = [p for p in state.players if p.car_id != player.car_id][0]
            return 1 if np.linalg.norm(player.car_data.position) < np.linalg.norm(opponent.car_data.position) else -1
        else:
            return 0
